...
const BuscarUsuariosPinsSinistroModel = require('../../models/rotinas/model_poppy_buscar_usuarios_com_pins_sinistro');
const ZerarPinsSinistroModel = require('../../models/rotinas/model_poppy_zerar_pins_sinistro_para_clientes');
const MoverPinsSinistroModel = require('../../models/rotinas/model_poppy_mover_pins_sinistro_para_purg');
const TransacoesPinsSinistroModel = require('../../models/rotinas/model_poppy_transacoes_pins_sinistro_para_purg');
const BuscarSaldosCarteirasModel = require('../../models/rotinas/model_poppy_buscar_saldos_carteiras');
const CarteiraPinsSinistroModel = require('../../models/rotinas/model_poppy_carteira_pins_sinistro_para_purg');
    ...
    async executeRecompraPinsSinistro(req, res) {
        ...
        try {
            ...
            if (idTokens.length === 0) {
                ...
                return res.status(200).json({
                    mensagem: 'Nenhum token válido encontrado.',
            ...
            if (Object.keys(tokensTotais).length === 0) {
                ...
                return res.status(200).json({
                    mensagem: 'Nenhum total válido encontrado.',
                    pinsSinistro: pinsSinistro,
                    usuariosPorToken: {},
                    tokensTotais: {},
                });
            ...
            for (const tokenId in tokensTotais) {
                ...
                for (const usuario of usuariosPorToken[tokenId]) {
                    ...
                    try {
                        ...
                        const resultado = await TransacoesPinsSinistroModel.transacoesPinsSinistro(
                        ...
                        return res.status(500).json({
                            ...
                            mensagem: `Erro ao processar o usuário ${usuario.usuario_id}: ${error.message}`,
            ...
            // Etapa 5: Movimentação dos tokens para purgação
            for (const tokenId in tokensTotais) {
                const { totalQuantidade, totalRendimento } = tokensTotais[tokenId];
                
                try {
                    const resultado = await MoverPinsSinistroModel.moverPinsSinistro(tokenId, totalQuantidade, totalRendimento);
                    ...
                    return res.status(500).json({
                        ...
                        mensagem: `Erro ao processar o Token ${tokenId}: ${error.message}`,
                        tokensTotais: tokensTotais,
                    });
                }
            }
            ...
            if (usuariosAFazerZerar.length === 0) {
                ...
                return res.status(200).json({
                    mensagem: 'Nenhum usuário para zerar tokens.',
                    pinsSinistro: pinsSinistro,
                    usuariosPorToken: {},
                    tokensTotais: tokensTotais,
                });
            ...
            if (!saldos.length) {
                ...
                return res.status(200).json({
                    mensagem: 'Nenhum saldo encontrado nas carteiras.',
                    pinsSinistro: pinsSinistro,
                    usuariosPorToken: {},
                    tokensTotais: tokensTotais,
                });
            ...
            for (const transacao of transacoesRealizadas) {
                ...
                try {
                    ...
                    const resultado = await CarteiraPinsSinistroModel.transacoesPinsSinistro(
                        usuario_id,
                        novoSaldo
                    );
                    ...
                    return res.status(500).json({
                        ...
                        mensagem: `Erro ao processar o usuário ${usuario_id}: ${error.message}`,
                        pinsSinistro: pinsSinistro,
                        usuariosPorToken: {},
                        tokensTotais: tokensTotais,
                    });
