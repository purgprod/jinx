const gerarTemplateConviteFamilia = ({ nomeGuardiao, linkAceitar, linkCadastro, temConta }) => {
    const link = temConta ? linkAceitar : linkCadastro;
    const botaoTexto = temConta ? 'ACEITAR CONVITE' : 'CRIAR CONTA E ACEITAR';
    const descricao = temConta
        ? `<strong>${nomeGuardiao}</strong> quer ser seu responsável na plataforma Purg e acompanhar sua jornada financeira.`
        : `<strong>${nomeGuardiao}</strong> te convidou para fazer parte da plataforma Purg como tutelado. Crie sua conta e aceite o convite.`;
